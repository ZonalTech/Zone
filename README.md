...
> **Install zonal from a normal shell, not from inside a virtualenv.** If a
> `.venv` is active, `py`/`pip` point at it and the install fails with
> *"Can not perform a '--user' install"*. Run `deactivate` first, or open a new
> terminal. zonal has **no dependencies**, so it installs cleanly into your
> real Python — no pipx needed.
...
`install.bat` runs `py -m pip install --user` into your real Python and tells
you the Scripts folder to add to `PATH` if needed.
...
> **Windows PATH note:** if `zonal` isn't found after install, its Scripts
> directory isn't on `PATH`. Add your user Scripts folder
> (`py -m site --user-base` → `…\Python3xx\Scripts`) to `PATH`, then reopen the
> terminal.
...
```bat
py -m pip install --user --upgrade path\to\zonal   :: update after pulling new CLI source
py -m pip uninstall zonal                           :: remove the CLI (zones on disk are untouched)
```
...
| Symptom | Fix |
|---|---|
| `Can not perform a '--user' install` / `No module named pipx` | A virtualenv is active in that shell (`py`/`pip` point at a `.venv`). Run `deactivate` (or `$env:VIRTUAL_ENV=$null`), or open a fresh terminal, then install into your real Python. |
| `zonal` not found after install | Add your user Scripts dir to `PATH` (`py -m site --user-base` → `…\Scripts`), then reopen the terminal. |
